...
function DurationPicker(
  ...
  function handleHoursUpdate(hours: number) {
    ...
    if (typeof m !== "string") {
      setPrepTimeInSeconds(util.durationToSeconds({hours, minutes: m}));
    } 
  }
  ...
  function handleMinutesUpdate(minutes: number) {
    ...
    if (typeof h !== "string") {
      setPrepTimeInSeconds(util.durationToSeconds({hours: h, minutes}));
    } 
  } 
  ...
  return (
    ...
    >
      <label>
        <input
          value={h.toString()}
          className="w-7 text-center rounded-md border border-indigo-100"
          type="number"
